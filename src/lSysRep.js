...
class LSystem {
  ...
  makeGetProduction(lhs) {
    return () => {
      if (lhs in this.productions) return this.productions[lhs];
      this.setProduction(this.__idProduction__(lhs)); return this.productions[lhs];
    };
  }
  ...
  get readableState() {
    return this.state.map((prodApp) => {
      const argsAux = (typeof prodApp.args !== 'undefined') ? prodApp.args.map((argFun) => argFun()) : [];
...
class Production {
  ...
  evaluateProduction(args, productions) {
    const paramsToArgs = zip(this.parameters, args);

    if (this.predicate(paramsToArgs)) {
      for (const pApp of this.rhs) {
...
class ProductionApplication {
  ...
  evalArgumentsInRHS(oldEnvironment) {
    ...
    const newEnvironment = this.evalArguments(oldEnvironment);
    const productionRHS = this.production.getRHS();

    return productionRHS.map((prodApp) => new ProductionApplication(prodApp.production(),
...
module.exports = {
  LSystem, Production, ProductionApplication, LExpression,
};
